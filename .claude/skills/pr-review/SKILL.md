...
**`minimal` is not a dial.** The trio is fixed *within* a mode: minimal runs the same three
...
**Mode.** A leading bare `minimal` selects minimal mode; anything else is full. It's a separate
axis from the target and composes with every row above — `/pr-review minimal`,
`/pr-review minimal <branch>`, `/pr-review minimal <repo>#<n>`. Everything downstream is shared:
...
`minimal` run to the trio because the change turned out hairy, and don't downgrade a full run
because the diff is two lines. If minimal was the wrong call, finish the pass and say so in the
...
Full mode only — in `minimal` skip this section entirely and use § Minimal mode instead.
...
## Minimal mode — one pass, in the main thread
...
2. **Select the rules.** Pipe that same changed-file list — all three parts of it — into
   ...
   one thing minimal must never do: the sub-repo's own rules aren't in your context either, so
...
**If minimal was the wrong call, say so rather than escalating.** A diff that turns out to be
...
the one guarantee minimal makes, and a half-review reported as done is worse than either mode.
