...
1. **`mt-scope`** — *investigate / discuss.* Establishes the cause and the difficulty read
   first, then 2–3 approaches whose load-bearing assumptions have been checked, tradeoffs,
   picks a direction, open questions. **No task breakdown, no file-level change plan** —
   stays divergent. → scope file. *(I approve the direction before planning.)*
...
- **`pr-review`** — the `reviewer` trio on demand: my diff, my branch, or a teammate's PR.
  ...
  I choose per run — full (three parallel subagents) or `mini` (the same three lenses in one
...
- 2026-08-13 — `scope` — **reshaped 4 beats → 5 so investigation precedes proposal.** The old flow ran its single research pass *before* any approach existed (its brief even said "what would each approach collide with"), then forbade a second one ("one research pass… don't loop") — so candidates were proposed unvalidated and got reversed mid-discussion. Now: **Frame** (restates the problem only — a ticket's stated cause is a hypothesis, not the frame) → **Investigate** (new beat: cause/mechanism + a **difficulty read** — shared vs local, consumer count, one repo or two, migrations, what's uncertain — **reported to me before any solution**, and the cheap moment to kill a bad premise) → **Candidates + validate** (name the one assumption that would collapse each candidate and check it; a killed candidate is a result) → **Discuss** → **Write file**. Research is now capped by **purpose, not time** ("only what would change a decision") — replacing "light / timeboxed / reconnaissance"; Explore agents scale to the idea instead of a fixed single medium-breadth agent, and grounding detail is kept (it's what the difficulty read is made of) and summarized only when writing the file. The no-descending guardrail now explicitly **permits naming code to explain a cause** (you can't state a mechanism without naming it) — the line is explaining vs planning, not vocabulary. Scope-file format unchanged: the difficulty read lands only as *justification* inside approaches/chosen direction, **never** a standalone estimate section plan would inherit as authority.

- 2026-08-18 — `scope` — **fixed how scope reports, and stopped it stalling between phases.** The investigation was fine; the message was a form. Two causes. (1) The skill had *no* presentation guidance — it says "stay at altitude" throughout, but that governs how deep to **think** (no task lists, no file-level plans), never how much to **show**; `response-altitude.md` was never referenced. What guidance existed made it worse: Discuss prescribed a per-candidate field list (what it is / tradeoffs / validation / difficulty), so three candidates rendered as a twelve-cell wall — the finding-card shape `response-altitude.md` bans. Now a **Reporting** section binds the skill to that rule and names the shape: lead with the cause, approaches as arguing prose rather than records, difficulty as a clause not a section, and **end with the decision being asked for** (a message that doesn't forces a round-trip — the failure it exists to prevent). (2) The 2026-08-13 checkpoints were implemented as hard turn boundaries (**"No approaches in this message"** + "two checkpoints are mandatory"), so scope stopped three times where two of the stops bought nothing. Ordering kept, blocking dropped: lead with the cause *within* the message and continue; stop early **only when the premise itself changed**, which is the one interruption worth its cost. One mandatory stop remains — I pick the direction before the file is written. Also retired the **beat** vocabulary: phases keep plain unnumbered names, the `← first checkpoint with me` marker is out of the heading, and numeric cross-refs ("go straight to beat 2") are gone — a skill that numbers and cross-references its own phases teaches the model to echo them as headers, which is half of why the output read as a form. Phase count, research cap, and the scope-file format are all unchanged.

- 2026-08-24 — `make-diagram` (new) + `plan` + `workflow` — **built the diagram skill, and made plan stop before implement.** Drains the `pipeline-flow-diagram-skill` task. Origin: a multi-consumer pipeline design that was unreadable as prose became obvious the moment it was drawn — module boxes carrying each function's `in :`/`out:` contract, the data flowing top-to-bottom, and `════╪════ ← the ONLY fork` marking where the consumers diverge. The skill encodes that grammar (`references/pipeline-fork.md` holds the worked CRMF-2202 example it was derived from) and, just as importantly, **owns the call on when *not* to draw** — under ~3 steps, a single linear consumer, or a list-not-a-flow all get a sentence instead. **ASCII by default and no language tag on the fence**: the output has to survive a plan file, a PR description, and a terminal, where mermaid is dead text; mermaid only on explicit request. **One shape ships**, with an explicit instruction to add new shapes as their own section rather than bending the pipeline grammar over them. Rejected en route: a broader `pr-explanation` skill that would have explained both plans and PRs — `pr-review` already opens with what-the-change-is, and both product repos own their `pr-description` skill, so the only real gap left ("explain a teammate's PR I'm not reviewing") is narrower than the original framing and stays deferred. Two consumers wired: `plan` reaches for it during the converge presentation (the moment I'm deciding whether the design is right), and `workflow` mentions it at the gate. **`plan → implement` is now a HARD GATE** (was "no gate — plan approval *is* the checkpoint"): approving a breakdown is agreeing the design is right, not saying start building, so plan closes by stopping and workflow waits for an explicit go-ahead. The conductor now enforces two gates, not one. Separately, plan's converge step **inverts write-and-review**: it used to render the whole plan into a chat message and hold the write until I approved — so I was reconstructing from prose a plan that was about to become a file anyway, and any revision meant re-rendering the whole thing. It now writes `<slug>-plan.md` first and reviews from the file, revising in place. One carve-out survives: a **genuine open question that isn't the whole plan** (a leftover judgment call, a direction that turned out ambiguous) is still asked on its own before the write — the failure mode being guarded against is dressing the entire plan up as a question in order to ask it.

- 2026-09-01 — `knowledge-base` (new, PR 1 of 4) — **the store and its retrieval wiring.** Design in `spec/knowledge-base-design.md`; drains the design half of the `cross-session-knowledge-base` task. `knowledge-base/` is gitignored (so a KB edit never needs a PR — accepted cost: no history, no backup) and holds `projects/` (where a project stands; throwaway once it ships) beside `concepts/<area>/` (durable, maintained) — **split because their lifecycles differ**: mixed together, neither one's freshness can be trusted. The mechanism that makes it work is **recognition, not search** — Claude cannot go looking for an entry it doesn't know exists, but it will open one whose line is already in context, which is exactly why `MEMORY.md` works. So `INDEX.md` is pulled into every session by an `@knowledge-base/INDEX.md` import in `CLAUDE.md` (nothing auto-loads a file for being *named* INDEX.md — the import is the whole mechanism), and grep over entry bodies is only the fallback for a line that failed to fire. Hence the one discipline the index file enforces in its own header: **lines are triggers, not summaries** — name what will be on screen (table names, error strings) rather than what the entry is about, because a line that only describes its subject never fires. Checked against the Claude Code docs: imports are recursive to 4 hops and `/context` lists loaded memory files, but **`.gitignore`-awareness and missing-target behavior are both undocumented** — the worst case takes all of `CLAUDE.md` down rather than just the KB, so `INDEX.md` must always exist (the `kb` skill will own that guarantee). Superseded prior art worth remembering: `spec/inline-computed-fields/CONTEXT.md`, **53 KB and loaded by nothing** — the failure mode the one-page cap exists to prevent. Still to come: the `kb` skill (2), the write gate making it the only write path (3, must follow 2), and consumers wiring `/done` + `scope`/`pr-review` (4).

- 2026-09-01 — `kb` (new, PR 2 of 4) — **the skill that owns the knowledge base.** Modes: `search` (grep entry bodies — the fallback for when no index line fired), `add`, `update`, and `graduate` (pull a finished project's durable residue into `concepts/` **before** its project doc is deleted; without that step, finishing a project deletes exactly what was worth keeping). Every write shows a diff and waits for a yes — behaving as if PR 3's hook already exists. Three invariants it checks on every touch: **`INDEX.md` must exist** (CLAUDE.md imports it and missing-target behavior is undocumented, so the bad case breaks all of CLAUDE.md, not just the KB), one line per entry in the index, and a **one-page cap** per entry — the cap being what stops the store drifting into codebase documentation, the way `spec/inline-computed-fields/CONTEXT.md` did at 53 KB. The skill's centre of gravity is deliberately the **index line, not the prose**: the index is the whole retrieval mechanism, so a line that fails to fire makes its entry invisible, and the rule is *name what will be on screen* (table names, error strings) rather than what the entry is about. Two things carried in from the design discussion rather than invented: **project progress is an explicit exception** to "don't store the derivable" (deriving it means reading every ticket and PR, and still misses what changed during implementation), and **staleness is Mohammad's call** — the skill flags a rot risk once and then defers, never refuses. Also added a feedback loop: re-deriving something the KB already holds is reported as an index-line bug rather than absorbed silently, since a silently-missed entry looks identical to a missing one.

- 2026-09-02 — `kb_write_gate_hook` + `doc-edit-diff-first` rule (PR 3 of 4) — **the write gate, built general enough to drain the `doc-diff-before-apply-rule` task too.** Two layers, deliberately. The **rule** covers any existing document (PRD, eng design, Notion page, spec, KB entry): present the change as a fenced ```diff block — `-` the author's original, `+` ours — section by section for a large doc, working from the original as base with unchanged prose byte-for-byte. The reason it's a rule and not just etiquette: **a document edit leaves no trace to review afterwards** — code gets a PR, a Notion page or a gitignored KB entry gets nothing, so if the change isn't seen before it lands it is never seen at all. The **hook** is the enforcement for `knowledge-base/**` specifically, matching by path *segment* (fires in worktrees; correctly ignores a similarly-named `knowledge-base-notes/`), failing open on malformed input. Decision is **`ask`, not `deny`** — deny would also block the `kb` skill, leaving no way in at all. **Unverified and worth knowing:** whether an `ask` decision actually prompts under bypass-permissions mode could not be tested from inside the authoring session, because hook config is snapshotted at session start (a mid-session settings edit didn't take effect — the probe write sailed through). Standalone logic is verified by piped JSON. If `ask` turns out to be suppressed, the gate is a silent no-op — the same worst case as PR 1's import — and the fallback is `deny` plus a marker-prefixed Bash escape, the pattern `test_scope_gate_hook` already uses. This is why the design is layered: the rule and the `kb` skill produce the behavior, the hook is only the backstop, so losing the backstop degrades rather than breaks it. Wiring note: hooks live in the repo but are wired in the **global** `~/.claude/settings.json`, so the wiring is never part of a PR; the new entry is `[ -f ]`-guarded so it stays a no-op until the file reaches the primary checkout on merge.

- 2026-09-02 — `done` + `scope` + `pr-review` (PR 4 of 4) — **the consumers, which is what turns the KB from a store into a habit.** `done` gains a **first** teardown step: update the project's `knowledge-base/projects/` entry with what this PR actually changed (scope that shifted, decisions taken while implementing), or `kb graduate <project>` if the project itself finished. It is deliberately step 1 because **step 2 deletes the scope and plan files** — the reasoning lives there, and after that it is unrecoverable. "Nothing worth keeping" is an allowed answer; an entry per PR is not the goal. On the read side, `scope` consults the index **before dispatching Explore agents** and `pr-review` **before dispatching the reviewer trio** — both placed ahead of the expensive step, which is the whole point, since re-investigating what the KB already holds is the cost the store exists to remove. `pr-review`'s framing is sharper than "look it up": findings from the entry go **into the lens briefs**, because a reviewer that doesn't know a design was deliberate reports it as a bug, and that false positive costs more than the lookup. Deliberately **not** wired: `plan`'s deep-research beat, which is arguably the biggest wide read of all — the design named scope and pr-review, and padding beyond it is the kind of scope creep `no-invented-requirements` exists to stop. Worth revisiting once the store has real entries. **Also this date: PR 1's import risk is RESOLVED** — a session's instruction re-read listed `knowledge-base/INDEX.md` with its content inlined, proving import resolution is a plain filesystem read and not git-aware. The missing-target case stays open, which is why `INDEX.md` must always exist.

- 2026-09-03 — `worktree` (PR 5, unplanned) — **fixed a hole PRs 1–4 shipped: the knowledge base was per-checkout, so it barely existed.** `knowledge-base/` is gitignored, and the only copy was ever created in the authoring worktree — so the **primary checkout had no `INDEX.md` at all** while its `CLAUDE.md` imported one, which is exactly the missing-target case that had been logged as the residual risk. Four of five existing worktrees were in the same state and `worktree_setup.sh` would have reproduced it for every future one. Worse than it sounds: **every funnel session runs in a worktree**, so the store would have been invisible precisely where it was meant to be used. Fix: **one store in the primary checkout, symlinked into each worktree** by the setup script (review trees included — they load `CLAUDE.md` too), plus a backfill of the seven live worktrees. A copy was rejected on purpose — it forks per worktree and whichever half got written wins by accident, a silent-divergence bug in a store whose entire purpose is being the one place a fact lives. Two things verified rather than assumed, because the target is now the only copy: `rm -r` over a directory containing a symlink removes the link and leaves the target intact (so `worktree_teardown.sh`'s closing `rm -r "$wt"` cannot destroy the store), and the write gate still fires through a link since `kb_write_gate_hook.py` matches the `knowledge-base` path *segment* without resolving symlinks. General lesson worth keeping: a gitignored path referenced by a tracked file is only as real as whatever creates it — the reference lands for every clone and worktree, the file does not.

- 2026-09-04 — `kb` + `kb_write_gate_hook` + `worktree_gate_hook` (reverts the 09-03 exemption) — **KB writes move to the shell, which is the actual fix.** The 09-03 exemption patched the wrong layer: underneath `worktree_gate_hook` sits **Claude Code's own session isolation**, which also resolves the symlink, sees the shared checkout, and refuses — and is not a hook, so it cannot be exempted. Root cause stated properly: **the KB is shared mutable state and worktrees exist to prevent exactly that**; three layers enforce "a worktree session must not write into the primary checkout" and all three resolve symlinks, as they must, or isolation would be one `ln -s` away from meaningless. The deeper mismatch is that *in-repo* in a worktree workflow means per-branch/isolated/ephemeral while a KB is single/shared/persistent. **Resolution: the store stays put; writes move to Bash.** Those guards police the `Edit`/`Write` *tools*, not the filesystem — policy about agent behaviour, not OS permissions — so `kb` writes via `cat > … <<'KB_EOF'` and the confirm gate moved onto the `Bash` matcher. `Edit`/`Write` on the store is now uniformly denied, which is desirable: it forces every write through the skill. Rejected: relocating the bytes outside the repo (cheaper, works, but moves the store out of mt-devkit — an explicit decision). Honest limit recorded in both the hook and the rule: recognising an arbitrary shell write is undecidable, so the matcher catches the skill's own shape plus obvious hand-rolled cases and will miss an exotic one — a backstop, not a wall. **Process lesson, which is the expensive part:** three fixes shipped before the real constraint surfaced, because each was verified against the layer just written rather than through the real path. A guard's behaviour is only established by exercising the actual operation in the actual context.

- 2026-09-04 — `kb_write_gate_hook` — **`ask` measured to be a no-op under bypassPermissions; the gate now denies with a visible escape.** The question open since PR 3 is closed, and the answer is the unwelcome one. Probe through the real path: hook live, wired on the `Bash` matcher, command matching its patterns — and the write executed with **no confirmation whatsoever**. Disambiguated by piping that exact command straight at the hook, which returned `permissionDecision: "ask"` — so it fired and the decision was swallowed by `defaultMode: "bypassPermissions"`. An `ask` gate in this setup is theatre. `deny` **is** honoured in that mode, observable all session from `worktree_gate_hook` blocking writes repeatedly. So the gate now denies by default and recognises one escape, a literal `MT_KB_WRITE=1` prefix, making the `kb` sequence diff → explicit yes → marked write. Honest about what it buys: the marker is added by the agent, so it is a declaration rather than a lock and cannot stop a determined write — but it makes an **incidental** one impossible, since nothing reaches the store without a deliberate token in plain sight in the command. Same shape as `MT_TEST_SCOPE_GATE=0`. **Generalisable:** a hook's decision *type* is not a free choice here — permission mode silently removes one of them, so any future gate wanting to interrupt rather than block should assume `ask` does nothing until proven otherwise on this machine.

- 2026-09-07 — `kb` + `CLAUDE.md` — **the write-timing rule the design specified but never shipped.** Reported symptom: the KB "constantly asks me to put stuff in it whenever I defer work or do deep dives." Investigation found **nothing configured** causing it — no hook, no rule, and the only wired write trigger is `done`'s teardown step, which is already the right moment. The pressure was **emergent**, from three always-loaded prose sources: the `kb` skill *description* (in every session's skill listing) advertising `add`/`update` as an anytime action; `CLAUDE.md`'s feedback-loop sentence, which is read-side but lands after a deep dive as "this store exists to prevent what you just did"; and § What earns an entry, a test of **content** with no timing qualifier — a deep dive passes it at the exact moment it happens. Meanwhile the design doc already carried the answer ("`concepts/` stays on explicit invoke to start — auto-writing concepts is where knowledge bases fill with junk") and it had never reached anything loaded at runtime. Evidence the timing was the defect and not the store: `INDEX.md` still read `_(none yet)_` under **both** headings five days in, so every one of those offers had been declined — a 0% acceptance rate is pure friction. Fix is prose in the three leaking places: a **When to offer a write** section in the skill naming two sanctioned moments (`done` close-out, explicit ask) and nothing else, the same clause added to the skill *description* since that is what is actually in context when the urge fires, and `CLAUDE.md`'s sentence retargeted so it can only read as "fix the index line". The non-funnel gap — a deep dive never reaches `done` — is closed by Mohammad, who runs `/done` on non-funnel sessions too, so there is no capture story needing an early ask. **Generalisable:** a skill's *description* is loaded every session while its body loads only on invoke, so a timing rule living only in the body cannot govern the moment the model decides to reach for the skill.
